...
- **Multi-tenancy** — the *Organization → Workspace → Agent* hierarchy, its RBAC role/permission model, and the `PermissionGuard` are implemented (migration 0051; "Project" was renamed **Workspace**). What is still missing is the tenant *experience*: a single default organization and workspace are seeded and there is no CRUD for either, enforcement ships in shadow mode (`AUTHZ_ENFORCE` opt-in), there is no invitation flow, and every user who signs in is provisioned into the default organization as *Owner* (there is no lower default role yet). Custom per-org roles are out of OSS scope.
