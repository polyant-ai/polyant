...
- **RBAC** — `Organization → Workspace → Agent` tenancy with four system roles (Owner / Admin / Member / Viewer) and a `resource:action` permission matrix; enforcement is opt-in via `AUTHZ_ENFORCE` (shadow mode by default)
...
- **Multi-tenancy** — the *Organization → Workspace → Agent* hierarchy, its RBAC role/permission model, and the `PermissionGuard` are implemented (migration 0051; "Project" was renamed **Workspace**). What is still missing is the tenant *experience*: a single default organization and workspace are seeded and there is no CRUD for either, enforcement ships in shadow mode (`AUTHZ_ENFORCE` opt-in), there is no invitation flow, every user who signs in is provisioned into the default organization as *Owner* (there is no lower default role yet), and the frontend has no tenant-scoped URLs (`/organizations/{org}/workspaces/{workspace}/...`). Custom per-org roles are out of OSS scope.
