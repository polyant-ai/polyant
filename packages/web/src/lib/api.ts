...
export const api = {
  ...
  mcpServers: {
    list: (slug: string) =>
      request<McpServer[]>(`/api/agents/${encodeURIComponent(slug)}/mcp-servers`),
    set: (
      slug: string,
      serverSlug: string,
      ...
      request<{ ok: boolean }>(
        `/api/agents/${encodeURIComponent(slug)}/mcp-servers/${encodeURIComponent(serverSlug)}`,
    ...
    test: (
      ...
      request<McpTestResult>(`/api/agents/${encodeURIComponent(slug)}/mcp-servers/test`, {
        method: "POST",
