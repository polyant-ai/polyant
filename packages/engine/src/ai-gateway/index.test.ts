...
describe("AI Gateway", () => {
  ...
  describe("chat", () => {
    ...
    it("passes conversationId, instanceId, and callType to logger", async () => {
      ...
      await chat(makeRequest(), { conversationId: "conv-1", instanceId: asAgentSlug("user-1") });
    ...
    it("passes callType 'service' to logger when specified", async () => {
      ...
      await chat(makeRequest(), { conversationId: "conv-1", instanceId: asAgentSlug("inst-1"), callType: "service" });
    ...
    it("emits the instance icon to the activity bus as a URL, never the raw base64 data URI", async () => {
      ...
      await chat(makeRequest(), { conversationId: "conv-1", instanceId: asAgentSlug("acme") });
    ...
    it("passes langsmith providerOptions to provider when langsmith config is present", async () => {
      ...
      await chat(
        ...
        { conversationId: "conv-1", instanceId: asAgentSlug("inst-1") },
      );

      expect(buildLangSmithProviderOptions).toHaveBeenCalledWith(
        { apiKey: "ls-key", project: "test-project" },
        { conversationId: "conv-1", instanceId: asAgentSlug("inst-1"), providerName: "openai", modelId: "gpt-4o" },
      );
      expect(mockProviderChat).toHaveBeenCalledWith(
  ...
  describe("chatStream", () => {
    ...
    it("passes callType through to logger in streaming mode", async () => {
      ...
      const stream = await chatStream(makeRequest(), { conversationId: "conv-1", instanceId: asAgentSlug("inst-1"), callType: "service" });
      await stream.response;
