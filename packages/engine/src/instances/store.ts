...
// lands, new agents land in the default workspace — the same place the
...
export async function listActiveInstances(): Promise<Agent[]> {
  return db.select().from(agents).where(eq(agents.status, "active")).then((rows) => rows.map(toInstance));
}
...
/** Find an agent by its UUID primary key. Returns undefined if not found. */
export async function findInstanceById(id: AgentUuid): Promise<Agent | undefined> {
...
/** Return all agents (any status), ordered by name (case-insensitive). */
export async function listAllInstances(): Promise<Agent[]> {
  return db.select().from(agents).orderBy(sql`LOWER(${agents.name})`).then((rows) => rows.map(toInstance));
}
...
export async function deleteInstance(slug: AgentSlug): Promise<boolean> {
  return db.transaction(async (tx) => {
    ...
    const convRows = await tx
      ...
      .from(conversations)
      .where(eq(conversations.agentId, slug));
    ...
    await tx.delete(conversations).where(eq(conversations.agentId, slug));
    await tx.delete(memories).where(eq(memories.agentId, slug));
    // knowledge_chunks cascade via their document_id FK.
    await tx.delete(knowledgeDocuments).where(eq(knowledgeDocuments.agentId, slug));
    // scheduled_task_runs cascade via their task_id FK.
    await tx.delete(scheduledTasks).where(eq(scheduledTasks.agentId, slug));
    // conversation_state is slug-keyed operational/PII data — drop it too.
    await tx.delete(conversationState).where(eq(conversationState.agentId, slug));
