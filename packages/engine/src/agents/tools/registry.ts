...
export interface ToolContext {
  ...
  /** Short-lived handoff of generated bytes between two tools of the same
   *  conversation (a plugin renders a PDF, another plugin uploads it). The
   *  engine owns the bytes and binds every handle to this conversation. */
  artifacts: import("./artifact-store.js").ArtifactApi;
}
...
// `ctx.artifacts`. Remove once a future SDK release adds the field itself.
