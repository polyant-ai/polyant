...
registerTool({
  ...
  description:
    "Post a message to Slack using the Slack channel configured on the current agent.\n" +
    ...
    "Returns a send confirmation, or an error if the Slack channel is not configured for this agent or the Slack API rejects the call.\n" +
    "Caveat: requires the `slack` channel to be enabled on the agent (`agent_channels`). Text supports Slack mrkdwn syntax (e.g. *bold*, _italic_, <url|label>).",
  category: "messaging",
  requiredSecrets: [
    {
  ...
  create: (ctx) => ({
    ...
    execute: async ({ channel, message }: { channel: string; message: string }) => {
      ...
      // Per-agent allowlist (opt-in). If the secret is set (non-empty after trim),
      ...
      if (rawAllowlist) {
        ...
        if (!allowed.includes(trimmedChannel.toLowerCase())) {
          ctx.audit.log({
            ...
            success: false,
            error: "Channel not in agent allowlist",
          });
          return { success: false, error: "Channel not in agent allowlist" };
        }
      }
