 ...
 * `@RequireRole()` or `@Public()`) is "undeclared" and is DENIED. There is no
 * shadow mode to fall back on, so forgetting the decorator breaks the route
 * loudly rather than leaving it open —
