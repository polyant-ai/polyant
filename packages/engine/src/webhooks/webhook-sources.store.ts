...
export async function listEventSources(slug: AgentSlug): Promise<EventSource[]> {
  ...
  return rows.map((r) => {
    try {
      return { ...r, instanceId: asAgentUuid(r.instanceId), config: JSON.parse(decrypt(r.config)) as Record<string, unknown> };
    } catch {
      webhookLog.warn("WebhookSources", `failed to decrypt config for source ${r.id}, using empty config`);
      return { ...r, instanceId: asAgentUuid(r.instanceId), config: {} as Record<string, unknown> };
    }
  });
}
...
export async function deleteEventSource(id: string, instanceId: AgentUuid): Promise<void> {
  await db.delete(eventSources).where(and(eq(eventSources.id, id), eq(eventSources.instanceId, instanceId)));
}

export async function rotateWebhookToken(id: string, instanceId: AgentUuid): Promise<string> {
...
export async function findByWebhookToken(
  token: string,
): Promise<{ source: EventSource; instanceId: AgentUuid; configReadable: boolean } | null> {
  const rows = await db
    .select()
    .from(eventSources)
  ...
  const rowInstanceId = asAgentUuid(rows[0].instanceId);
  return { source: { ...rows[0], config, instanceId: rowInstanceId }, instanceId: rowInstanceId, configReadable };
}

async function verifyEventSourceOwnership(eventSourceId: string, instanceId: AgentUuid): Promise<boolean> {
  const rows = await db
...
export async function listDefinitions(eventSourceId: string, instanceId: AgentUuid): Promise<EventDefinition[]> {
...
export async function deleteDefinition(id: string, eventSourceId: string, instanceId: AgentUuid): Promise<void> {
  if (!(await verifyEventSourceOwnership(eventSourceId, instanceId))) return;
