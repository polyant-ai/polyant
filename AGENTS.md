...
All user messages are handled by a single **Supervisor** agent that has access to a per-instance set of tools (memory, web search, knowledge, channel-specific integrations, ...). For complex multi-step work, the Supervisor can spawn isolated sub-agents.

```
User Message
    |
    v
[Instance Resolution] (look up instance config from PostgreSQL)
    |
    v
...
The set of tools available to a given instance is determined by the rows enabled in the `instance_tools` table. Sub-agents inherit those tools **except** `spawnTask`, to prevent infinite recursion.
...
The system prompt is assembled from 8 modular sections stored in the `instance_prompts` PostgreSQL table (one row per `(instanceId, sectionKey)`). Defaults for new instances live in `packages/engine/src/instances/defaults.ts` and are seeded on instance creation:
...
Sections can be customized per-instance from the admin panel (`PATCH /api/instances/:slug/prompts`) or directly in the database. The Supervisor reads them via `getPrompts(instanceId)` from `prompts.store.ts` (60s TTL cache).

Available tools are filtered per-instance via the `instance_tools` table (auto-recomputed when skills change). Tool registration is automatic at boot: every `*.tool.ts` file in `packages/engine/src/agents/tools/` calls `registerTool()` and the supervisor queries `getToolRegistry()` — no hardcoded tool imports.
...
All tools are defined in `packages/engine/src/agents/tools/` as self-registering `*.tool.ts` files. To add a new tool, create a new file that calls `registerTool()` — the registry picks it up at boot and the `tools` DB table is synced automatically. Per-instance enablement is then controlled via `instance_tools`.
