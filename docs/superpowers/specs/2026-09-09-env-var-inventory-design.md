...
| `PLATFORM_S3_BUCKET` `_REGION` `_ACCESS_KEY_ID` `_SECRET_ACCESS_KEY` | One bucket for every tenant on the installation, which is the wrong tier for a thing a tenant owns — and no deployment had ever set them, so no attachment was ever stored anywhere and the panel's attachment view could not render. Storage is now the agent's, from the secrets `fileUpload` already declared, resolved in `attachments/agent-s3.ts`. The `retention` gap is unchanged and known: stored objects outlive the conversation rows |
