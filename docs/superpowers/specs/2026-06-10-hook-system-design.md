...
- **Table `hook_executions`** — slug-keyed telemetry, same trust class as `pipeline_traces`/`tool_audit_logs`: `instance_id`/`conversation_id` as `text`, `hook_id` as a plain `uuid` (intentionally NOT a FK — history outlives the hook), `event`, `action_type`, `tool_name`, `success`, `error`, `duration_ms`, `args` (jsonb, rendered post-template input), `result` (text, JSON-stringified output truncated at 4000 chars), `created_at`. Index on `(conversation_id, created_at)`. Storing args/result matches the exposure class of message `steps` (which already persist LLM tool args/results verbatim); the executor reports them via a capture callback so the input survives failures and timeouts. The global audit log still records outcome only.
...
- **Playground (live)** — `runHooks()` returns a `HookExecutionSummary[]`; the pipeline threads them onto `StreamOutgoingMessage` (pre hooks synchronously, post hooks on the `completed` promise) and the typed SSE endpoint emits `hook-execution` events: pre-LLM hooks right after the stream opens, post-LLM hooks before `done` (which now awaits the synchronous post-processing phase). The playground renders them as pills on the assistant turn. Live-only: loaded conversations show executions in the Conversations page instead.
