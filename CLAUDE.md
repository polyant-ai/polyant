...
- **Model capability catalog**: `packages/engine/src/ai-gateway/model-catalog.ts` is the single source of truth for per-`(provider, model)` metadata — pricing (`input/output/cacheRead?/cacheWrite?`) + capabilities (`reasoning/reasoningAlwaysOn?/vision/temperature/cache`). Adding a model = ONE entry with every field filled in. The gates (`isThinkingCapable`, `modelSupportsVision`, `temperatureSupported`, `cacheSupported`, `isReasoningAlwaysOn`, `isReasoningAdaptive`) and `estimateCost*` are LOOKUPS into it. The per-provider regexes in `config.ts` (`*Fallback` fns) + `vision.ts`'s `visionCapableFallback` survive ONLY as a logged fallback for un-catalogued ids (Bedrock/Nebius drift) — `warnCatalogFallback` logs once per (gate, provider, model). `model-catalog.test.ts` guards catalog integrity + that catalogued rows still match the fallback heuristic. **Capability values are LIVE-VERIFIED against the provider APIs** (OpenAI/Anthropic/Bedrock probed directly; corrections applied — e.g. o3/gpt-5.6 `reasoningAlwaysOn`, Bedrock haiku/minimax `reasoning`, gpt-5.4/4.1 cache-read rates; Nebius left on console modality tags — its OpenAI-compatible endpoint is too permissive for accept/reject probing). **Reasoning control IS in the catalog now**: `reasoningAdaptive` marks the newer Claude models (Opus 4.7/4.8, Sonnet 5, Fable 5) that require `thinking.type:"adaptive"`+`effort` and REJECT the legacy `enabled`+`budgetTokens` shape (a 400 — this had broken extended thinking on those flagship models on both Anthropic 1P and Bedrock). The ai-gateway builds the per-model thinking payload from `isReasoningAdaptive`, and every provider now applies the per-instance `thinkingLevel` (OpenAI/Anthropic previously hardcoded it); the web Settings reasoning-level select shows for any thinking-capable model (was Nebius-only)
