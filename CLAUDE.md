...
- **Nebius Token Factory is an OpenAI-compatible chat provider** (`ai-gateway/providers/nebius.ts`, `provider="nebius"`). No bespoke adapter: it wraps `@ai-sdk/openai-compatible`'s `createOpenAICompatible({ baseURL: "https://api.tokenfactory.nebius.com/v1", apiKey, includeUsage: true })` inside the shared `createProvider`. Per-instance key is the `nebius_api_key` secret (`SECRET_KEYS.NEBIUS_API_KEY` → `config-resolver` `apiKeys.nebius` → `ChatRequest.apiKeys.nebius`). **Reasoning is a property of the model, not a request flag** — reasoning models emit `reasoning_content`, which `@ai-sdk/openai-compatible` maps to reasoning blocks automatically; nothing is injected into `providerOptions` (unlike Anthropic/OpenAI in `resolveCallConfig`). The `providerConfigs.nebius` catalog in `config.ts` mirrors the account's live `GET /v1/models` (there is NO pricing/capability API — prices come from the console prices page, capability from the console modality tag, NOT the model name: e.g. `Nemotron-3-Nano-Omni` is text-only, `Cosmos3-Super-Reasoner`/`Kimi-K2.6` are vision). Reasoning gate: the `nebius` case in `isThinkingCapable`; vision gate: `VISION_CAPABLE` in `vision.ts`. **Version pin (build-breaking):** use `@ai-sdk/openai-compatible@^2.0.56` — it depends on `@ai-sdk/provider@3.x` (spec `LanguageModelV2/V3`), matching the repo's `ai@6`. The `3.0.x` line depends on `@ai-sdk/provider@4` (`LanguageModelV4`) and fails typecheck (`"v4" not assignable to "v2"`). Re-align this pin whenever `ai` is bumped.
