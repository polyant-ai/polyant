...
- **Prompt caching + cache-aware cost tracking** — the supervisor prompt is split so provider prompt-caching is effective. `buildSupervisorSystemPrompt()` returns `{ system, turnContext }`: `system` is the STABLE prefix (per-instance DB sections 1–7 + any persisted webhook `contextPrompt`), `turnContext` is the PER-TURN volatile block (current datetime, channel identity, running summary, conversation state, opt-out hint). The supervisor injects `turnContext` at the **tail of the messages** — wrapped in a `<context>…</context>` block prepended to the current user message (`buildUserContent`) — so it never invalidates the cached system/history prefix. Previously the minute-granular datetime lived in `system` and broke caching on every minute boundary. The skills query is `.orderBy(skills.slug)` so the skills list never reorders inside the cached section. **Anthropic** cache breakpoints are injected via a `prepareMessages` hook on `createProvider` (`providers/anthropic.ts` → `applyAnthropicPromptCaching`): the folded `system` string is moved into a leading `role:"system"` message carrying `providerOptions.anthropic.cacheControl = { type: "ephemeral" }` (caches tools+system — the dominant win, since system is byte-identical across every conversation of an instance), plus a second breakpoint on the last history message (from the 2nd turn onward, so single-turn conversations never pay a wasted history cache-write). OpenAI caches automatically (no params); **Bedrock-Anthropic uses a different mechanism (`cachePoint`) and is NOT yet wired — follow-up**. Cost/usage: the AI SDK v6 `inputTokenDetails.{cacheReadTokens,cacheWriteTokens}` breakdown is extracted in `mapUsage`/`buildUsage` into `TokenUsage.{cachedInputTokens,cacheCreationInputTokens}` (subset of `promptTokens`); `estimateCost` re-prices cache reads at a reduced rate and Anthropic cache writes at a premium via per-provider multipliers (`CACHE_MULTIPLIERS`: anthropic/bedrock read 0.1× / write 1.25×; openai read 0.5× / no write); `ai_logs` gained `cached_input_tokens` + `cache_creation_input_tokens` (migration 0062). **Caveat**: Anthropic caching has a write cost (break-even ~2 reads on the same prefix within the 5-min TTL) — measure **net cost per conversation** (read + write), not just hit-rate, and validate the exact SDK usage semantics against a live provider before drawing conclusions.
