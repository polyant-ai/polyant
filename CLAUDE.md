...
- **Embeddings are per-instance, provider-aware.** Each instance has `embedding_dim` (1024 or 1536). OpenAI-provider instances can use either dim; Bedrock-provider instances are always 1024 (Titan v2). Anthropic-provider instances fall back to OpenAI for embeddings and must configure `openai_api_key`. The `embeddings-gateway` module picks the right model + credentials based on the instance. The `memories` and `knowledge_chunks` tables have two parallel vector columns (`embedding vector(1536)`, `embedding_1024 vector(1024)`) with an XOR check constraint — exactly one is populated per row, decided by the owning instance's `embedding_dim`. The re-embed job (`reEmbedInstance`) migrates BOTH `memories` and `knowledge_chunks`, and flips `embedding_dim` to 1024 only once NO legacy 1536-dim rows remain in EITHER table (prevents an instance with 0 memories but legacy knowledge chunks from flipping early and silently dropping those chunks to FTS-only search). A provider switch auto-triggers re-embed on the server (`shouldReEmbedAfterSwitch` in the PATCH handler), not only via the UI dialog — so direct Management-API callers are covered too. `computeMemoryStatus` validates provider/`embedding_dim` compatibility (via `SUPPORTED_DIMS`): a stranded state like bedrock + 1536 reports `canEnable: false` rather than falsely healthy.
