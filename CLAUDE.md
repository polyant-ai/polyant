...
│   │       └── <instanceId>/conversations/<convId>/  # readFile/writeFile/github:cloneRepo scratch dir
│   │
...
- The `workspaces/` directory holds **only** per-conversation tool sandboxes (`workspaces/<id>/conversations/<convId>/`) used by `readFile` / `writeFile` / `github:cloneRepo` (the last now shipped as the `polyant-github-plugin` plugin, not a core tool)
