...
- **Model capability catalog**: `packages/engine/src/ai-gateway/model-catalog.ts` is the single source of truth for per-`(provider, model)` metadata — pricing (`input/output/cacheRead?/cacheWrite?`) + capabilities (`reasoning/reasoningAlwaysOn?/vision/temperature/cache`). Adding a model = ONE entry with every field filled in. The gates (`isThinkingCapable`, `modelSupportsVision`, `temperatureSupported`, `cacheSupported`, `isReasoningAlwaysOn`) and `estimateCost*` are LOOKUPS into it. The per-provider regexes in `config.ts` (`*Fallback` fns) + `vision.ts`'s `visionCapableFallback` survive ONLY as a logged fallback for un-catalogued ids (Bedrock/Nebius drift) — `warnCatalogFallback` logs once per (gate, provider, model). `model-catalog.test.ts` guards catalog integrity + that catalogued rows still match the fallback heuristic. NOT yet in the catalog: the reasoning *control mechanism* (Anthropic `budgetTokens` / OpenAI+Bedrock effort / Nebius `enable_thinking`) still lives per-provider in `providers/*.ts` — follow-up (#189 split note)
