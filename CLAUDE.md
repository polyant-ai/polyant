...
Open-source platform for building AI assistants with long-term memory, multi-channel support, and per-agent customization. TypeScript/Node.js (ESM). npm workspaces monorepo.
...
- **packages/engine** (AI runtime + management API):
  ...
  - Encryption: AES-256-GCM (Node.js crypto) for skill env vars and agent secrets
...
│   │   │   ├── instances/            # Agent CRUD + secrets + channels + config resolver (dir not yet renamed)
│   │   │   ├── skills/               # Global skill library (SkillsService + controller)
│   │   │   ├── room/                 # Proactive agent event-driven loop
│   │   │   │   ├── room.schema.ts   # 3 tables: agent_room, event_definitions, room_activity_log
...
│   │   │   │   ├── openai/           # /v1/chat/completions, /v1/models, /api/agents/:slug/chat/stream (typed SSE)
│   │   │   │   ├── instances/        # /api/agents CRUD + secrets + channels (dir not yet renamed)
│   │   │   │   ├── conversations/    # /api/conversations
│   │   │   │   ├── analytics/        # /api/analytics + /api/agents/:slug/analytics
│   │   │   │   ├── tools/            # /api/tools (read-only catalog)
│   │   │   │   ├── room/            # /api/agents/:slug/room + /webhooks/:token
│   │   │   │   └── memories/         # /memories
...
│   │       └── <agentId>/conversations/<convId>/  # readFile/writeFile/gitCloneRepo scratch dir
│   │
...
## Agent Configuration (Database-First)

**All agent configuration is stored in PostgreSQL — NOT on the filesystem.**

| Config | DB Table | Notes |
|--------|----------|-------|
| Prompts (8 sections) | `agent_prompts` | Seeded from `instances/defaults.ts` on create |
| Skills (global catalog) | `skills` + `skill_versions` | CRUD via `/api/skills` |
| Skills (per-agent) | `agent_skills` | Enable/disable, version pinning, env vars |
| Tools (global catalog) | `tools` | Self-registered at boot from `*.tool.ts` |
| Tools (per-agent) | `agent_tools` | Auto-recomputed when skills change |
| Secrets | `agent_secrets` | AES-256-GCM encrypted |
| Channels | `agent_channels` | Telegram, Slack, WhatsApp config |
| Room config | `agent_room` | One-to-one with agent, prompt + outbound channel |
...
| Hooks | `agent_hooks` | Lifecycle event → action (v1: run tool), template args, per-event ordering |

**IMPORTANT — DO NOT use filesystem for any agent configuration:**
- Prompts are read via `getPrompts(agentId)` from `prompts.store.ts` (60s TTL cache)
- Skills are discovered via DB joins in `supervisor/prompt.ts` → `discoverSkills()`
- Tool enablement is resolved via `getEnabledToolNames()` from `agent-tools.store.ts`
...
- There is no `_template/` directory — new agents are seeded from DB defaults (`instances/defaults.ts`)
...
**`hubspotContact` supports custom properties:** the tool accepts a generic `customProperties: Record<string, string>` parameter for create/update (to write HubSpot custom properties like `evento`), and `filters` + `returnProperties` + `limit` + `after` for search (to query and paginate by custom property). The tool never hardcodes property names — agent-specific values (like an event name) live in the agent prompt, not in the tool code.

**`slackPostMessage` tool:** framework-first, generico (`packages/engine/src/agents/tools/slack-post-message.tool.ts`). Accetta `channel` (nome `#nome`, ID canale `C...` o ID utente `U...`) e `message`. Usa le credenziali del canale Slack configurato sull'agente corrente via `channelManager.sendOutbound(agentSlug, "slack", channel, message)` — lo Slack adapter risolve automaticamente gli ID utente aprendo un DM. Il canale di destinazione vive nel prompt dell'agent (no config extra). Il body del messaggio NON è loggato in audit (solo la lunghezza) per evitare leak di PII.
...
**When modifying prompts:** use the Management API (`PATCH /api/agents/:slug/prompts`) or update `agent_prompts` rows. Default prompt content for new agents is defined in `packages/engine/src/instances/defaults.ts`.
...
- **Config via Zod**: all env vars parsed and validated in `packages/engine/src/config.ts`. Never read `process.env` directly elsewhere (documented exceptions: `DEFAULT_INSTANCE_ID`, `SANDBOX_ROOT` — with a deprecated `WORKSPACES_ROOT` fallback during the rename deprecation window, `LOG_LEVEL`). Altre letture intenzionali (filtri subprocess env, default params per testabilità, tool registry `requiredEnv` discovery) sono marcate con commento `// CONVENTION-EXCEPTION:` nel codice e devono restare circoscritte a quei pattern
...
- **Channel adapters are per-agent**: configs stored encrypted in DB, started/stopped dynamically via admin panel or API
...
- **GDPR opt-out (STOP/START)**: deterministic keyword gate runs at two chokepoints — a pre-LLM inbound gate at the top of `handleMessage`/`handleMessageStream` (short-circuits opted-out contacts; `runOptoutGate` in `packages/engine/src/optout/optout-gate.ts`) and outbound suppression inside `channelManager.sendOutbound`/`sendOutboundTemplate` (blocks proactive sends, coordinator bypasses with `skipOptoutCheck`). Opt-out state persisted per `(agentId, channelType, channelId)` in `contact_optouts` table (cascade on agent delete, NOT on conversation delete). Config lives as six columns on `agents` (opt-out enabled, stop/resume keywords, closing/resuming messages, prompt-hint enabled). The LLM is never the enforcer; the keyword is injected into the supervisor prompt purely as informational context. Admin endpoints: `GET/POST/DELETE /api/agents/:slug/optouts`. v1 limitation: STOP as a reply to a Room broadcast is not honored. Spec: `docs/superpowers/specs/2026-06-11-gdpr-optout-design.md`
...
- **Embeddings are per-agent, with an INDEPENDENT embedder provider.** The embedder is a dedicated `agents.embedding_provider` column (`openai` | `bedrock`; Anthropic has no embeddings API), chosen **separately from the chat `provider`** — changing the chat LLM never touches embeddings (migration 0052 backfilled `embedding_provider` from the chat provider so existing agents were unaffected). Each agent also has `embedding_dim` (1024 or 1536): OpenAI can emit either, Bedrock Titan v2 is always 1024. `embeddingProviderFor(provider)` survives only as the default at agent creation and for the migration backfill — it is NOT the runtime source of truth. `provider-resolver.ts` resolves the embedder model + credentials from `embedding_provider` (an `openai` embedder needs `openai_api_key` even when chat is bedrock/anthropic; a `bedrock` embedder needs AWS creds even when chat is openai). The admin Settings tab exposes a dedicated **Embedder** select. The `memories` and `knowledge_chunks` tables have two parallel vector columns (`embedding vector(1536)`, `embedding_1024 vector(1024)`) with an XOR check constraint — exactly one is populated per row, decided by `embedding_dim`. `computeMemoryStatus` validates `embedding_provider`/`embedding_dim` compatibility (via `SUPPORTED_DIMS`): a stranded state like bedrock + 1536 reports `canEnable: false` rather than falsely healthy.
- **Changing the embedder provider is destructive — no conversion/migration.** Embeddings produced by different providers live in incompatible vector spaces, and re-embedding existing data is slow, costly, and error-prone. So switching an agent's `embedding_provider` (openai↔bedrock) does NOT convert existing vectors — it **wipes** them. `resetEmbeddingsForProviderSwitch(slug, uuid, newEmbeddingProvider)` (in `embeddings-gateway/embedding-reset.service.ts`) deletes ALL memories and the ENTIRE knowledge base (documents + chunks, raw content included) and realigns `embedding_dim` to the new provider's default, in one transaction. **The deletes are keyed by the agent SLUG** (the `memories`/`knowledge_*` `agent_id` columns store the slug, not the UUID); the `agents` row update is keyed by the UUID. Passing the UUID to the slug-keyed deletes was a bug that left data orphaned while only realigning the dim — the function now takes both identifiers, branded, so the compiler prevents recurrence. Conversations are untouched — only extracted memories are removed, so keyword (FTS) search over raw messages keeps working. The `PATCH /api/agents/:slug` handler detects the switch with `embeddingProviderChanged` (which now compares `embedding_provider`, not the chat provider) and **rejects it with 400 when there is data to lose unless the body carries `confirmWipe: true`** — protecting scripted Management-API callers from silent data loss. The admin UI shows a destructive-confirmation dialog and sends `confirmWipe` after the user accepts; the PATCH response carries the `wiped` counts. The KB raw text survives a switch via **export/import** (below): export before switching, then re-import to re-embed at the new dim. Memories are not recoverable this way (they are LLM-extracted).
- **Knowledge-base export/import (JSON).** `GET /api/agents/:slug/knowledge/export` returns a JSON bundle `{ version, agentSlug, documents: [{ filename, content, … }] }` (every document with its raw content). `POST /api/agents/:slug/knowledge/import` bulk-creates documents from such a bundle and re-embeds each with the agent's CURRENT embedder. Filename collisions are resolved by appending a progressive suffix (`manuale.txt` → `manuale (1).txt`), never overwriting; the bundle is validated up front (size, non-empty, per-agent cap) so the import is all-or-nothing, and the embedder must be configured before any document is created. The `export` route is declared BEFORE `:docId` so it is not captured as a document id. Web Knowledge tab has Export/Import buttons.
...
- **Memory extraction is conditional** on the agent's `memoryEnabled` flag. The extraction prompt includes today's date and converts relative dates to absolute. Facts are written in the same language as the conversation.
...
- **Event source operations are agent-scoped**: all event source and definition mutations verify ownership via `agentId` — event sources directly in the WHERE clause, definitions via `verifyEventSourceOwnership()` which confirms the parent event source belongs to the agent. The `mark_events_completed` harness tool also scopes by `agentId`. This prevents IDOR across agents
- **`gitCloneRepo` credential lifecycle (#87)**: the GitHub token and the credential helper are written to `.git/polyant-token` (mode 0600) and `.git/polyant-askpass.sh` (mode 0700) inside each cloned sandbox so that subsequent git operations (push/fetch by Claude Code) can authenticate. Both files are removed automatically by `cleanupRepo()` when the conversation ends and by `cleanupStaleRepos()` (stale threshold: 2h). **Trade-off**: while the sandbox exists, the token is at rest on disk. Sandboxes must be treated as ephemeral state: never backup/rsync/tar/commit them, never expose `workspaces/<agentId>/` via any external share. A warning is logged if a leftover `.git/polyant-token` is detected during stale cleanup — that signals a crashed prior run
...
- **Completion API auth**: `/v1/chat/completions` uses per-agent API keys (existing mechanism: `agent_secrets` + `authEnabled` flag). NOT session-based
...
The project supports multiple AI assistant **agents**, each with its own configuration:

- **Agent = shared assistant configuration**: an agent defines personality, tools, skills, and can serve multiple users. Identified by `agentId` throughout the codebase (DB columns: `agent_id`)
- **Branded identifiers (`AgentSlug` vs `AgentUuid`)**: the two agent identifiers are distinct branded string types in `packages/engine/src/instances/identifiers.ts`. The human-readable `slug` (`agents.slug` — the external contract: API `model` field, URL `:slug`, channel keys, sandbox dirs, `conversationId` prefix) is `AgentSlug`; the `agents.id` UUID and the FK columns referencing it are `AgentUuid`. Slug-text tables (memories, conversations, knowledge, scheduled_tasks, audit, traces, ai_logs) store the slug → their store functions take `AgentSlug`; uuid-FK tables (room, prompts, secrets, channels, agent_skills, agent_tools, skill_env, webhooks/event sources + event_backlog) store the UUID → their store functions take `AgentUuid`. The ONLY sanctioned slug↔UUID conversion is `resolveAgentId(slug)` / `resolveAgentSlug(id)` in `resolve-agent-id.ts`. `asAgentSlug` / `asAgentUuid` are zero-cost (runtime identity) casts — use them ONLY at trusted boundaries (DB row reads, validated `@Param`, config constants). A plain `string` is not assignable to a brand, so the compiler now prevents the silent "zero rows" mismatch that used to occur when a slug reached a UUID query (or vice versa)
- **DB table `agents`**: metadata (slug, name, description, status, memoryEnabled, langsmithEnabled, authEnabled)
- **OpenAI-compatible API**: exposes agents as models — clients select an agent via the `model` field
...
- **Agent CRUD**: `GET/POST/PATCH/DELETE /api/agents` + `GET /api/agents/models`
- **Prompt management**: `GET/PATCH /api/agents/:slug/prompts`
- **Tool management**: `GET/PATCH /api/agents/:slug/tools`
- **Skill management**: `GET/PATCH /api/agents/:slug/skills` + skill env vars (`GET/PUT/DELETE`)
- **Secrets management**: `GET/PUT/DELETE /api/agents/:slug/secrets`
- **Channels management**: `GET/PUT/DELETE /api/agents/:slug/channels/:type`
- **Conversations**: `GET/DELETE /api/conversations` + messages
- **Memories**: `GET/POST/DELETE /memories`
- **Analytics**: `GET /api/analytics` + `GET /api/agents/:slug/analytics`
- **Global skill library**: `GET/POST/PUT/DELETE /api/skills`
- **Tool catalog**: `GET /api/tools` (read-only registry list)
- **Room**: `GET/PUT/DELETE /api/agents/:slug/room` + event sources CRUD + definitions CRUD + backlog query + activity log
- **Hooks**: `GET/POST/PATCH/DELETE /api/agents/:slug/hooks`
- **Webhooks**: `POST /webhooks/:webhookToken` — external event ingestion with LLM-based matching

### Admin Panel (implemented)

- Agent management (CRUD, provider/model selection, status toggle)
- Prompt editing (8 sections with accordion UI)
- Tool toggling (enable/disable per agent)
- Skills management (enable/disable, env var configuration with encryption)
- Settings tab (per-agent API keys, auth toggle, LangSmith toggle, memory toggle)
- Channels tab (Telegram, Slack, WhatsApp config with encrypted storage)
- Playground page (chat with agents, auth token persistence)
- Analytics dashboard (KPI graphs, per-agent metrics)
