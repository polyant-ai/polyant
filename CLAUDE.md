...
**Gateway mode carries no tenancy, and is incompatible with `AUTHZ_ENFORCE=true`.** `parseAlbOidcData` cannot resolve `orgId` (there is no local `users` row to map the Cognito `sub` onto), so a gateway-forwarded principal has neither an org claim nor any `role_bindings`. Two consequences: (1) `TenantService` falls back to the **default organization** for any principal without `orgId`, so the tenant-scoped admin panel still resolves its URLs — without that fallback every page would show a "sign in again" prompt that no re-login can satisfy; (2) under `AUTHZ_ENFORCE=true` a gateway principal is denied on every `@RequirePermission` route (`resolveScope` derives no scope), which is not a bug to patch at the guard but the missing gateway-identity → local-user mapping. Run gateway mode in shadow mode until that mapping exists.
...
- **Guard**: `PermissionGuard` is `APP_GUARD` #3 (after `ThrottlerGuard`, after `AuthGuard`), driven by `@RequirePermission()`. **SHADOW MODE IS THE DEFAULT** — with `AUTHZ_ENFORCE` unset every would-be denial is logged and downgraded to allow; only `AUTHZ_ENFORCE=true` fails closed (including on routes that declare no permission). Do not assume a route is enforced in a default deployment; the RBAC e2e suite (`packages/web/e2e/rbac/`) boots the engine with `AUTHZ_ENFORCE=true` for exactly this reason. **Three declarations satisfy the guard**, and `route-authorization-guardrail.test.ts` accepts exactly the same three (keep the two in lockstep — its `ALL_CONTROLLERS` list must name EVERY registered controller; it once omitted `MeController`/`UsersController`/`CredentialsController`/`SkillsController` and so hid seven handlers that 403 under enforcement): `@RequirePermission()` (RBAC), `@RequireRole()` (deployment-level role — `RoleGuard` runs first and hard-denies with no shadow mode, so `/api/users` is gated without an RBAC scope), and `@SelfService()` (the caller acts on their own identity, so no scope applies — `POST /api/me/password`; it is NOT `@Public()`, AuthGuard still runs). Anything else fails closed under enforcement
